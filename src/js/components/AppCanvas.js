...
function AppCanvas(props) {
  ...
  const refChangeModel = useRef(null);
  const refLoading = useRef(null);

  const toggleLoading = (isLoadingVisible) => {
    refLoading.current.style.display = (isLoadingVisible) ? 'block' : 'none';
    refChangeModel.current.style.display = (!isLoadingVisible) ? 'block' : 'none';
    refAdjustEnter.current.style.display = (!isLoadingVisible) ? 'block' : 'none';
  }
  ...
  return (
    <div>
      <div className='header'>OCHO VTO</div>
      <div ref={refPlaceHolder} className='JeelizVTOWidget'>
        ...
        <div ref={refChangeModel} className='JeelizVTOWidgetControls JeelizVTOWidgetChangeModelContainer'>
          ...
          <button className='JeelizVTOWidgetButton' onClick={load_modelBySKU}>by SKU</button>
        </div>
